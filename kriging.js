...
var kriging = function() {
	...
	for(i=0,k=0;i<n;i++)
		...
		distance[k][0] = Math.pow(
		    ...
		    Math.pow(y[i]-y[j], 2), 0.5);
                */
                distance[k][0] = distVincenty(x[i],y[i],x[j],y[j]);
	...
	for(i=0;i<n;i++) {
	    for(j=0;j<i;j++) {
                ...
                */
		K[i*n+j] = variogram.model(distVincenty(x[i],y[i],x[j],y[j]),
					   variogram.nugget, 
					   variogram.range, 
					   variogram.sill, 
					   variogram.A);
	...
	for(i=0;i<variogram.n;i++)
	    ...
	    k[i] = variogram.model(Math.pow(Math.pow(x-variogram.x[i], 2)+
            ...
            */
            k[i] = variogram.model(distVincenty(x,y,variogram.x[i],variogram.y[i]),
                                variogram.nugget, variogram.range, 
                                variogram.sill, variogram.A);
